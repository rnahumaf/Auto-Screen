...
export async function renderScreenProject(project: CaptureProject, options: RenderOptions): Promise<RenderResult> {
  ...
  const cursorMode = project.capture.cursorMode ?? (project.capture.drawMouse ? "native" : "hidden");
  const normalizedProject: CaptureProject = { ...project, capture: { ...project.capture, cursorMode } };
  ...
  graph.push(`[sped]${cameraPlan.filter}[camera]`);
