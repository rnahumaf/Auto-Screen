...
- Cursor: `software` captura sem cursor nativo e registra diretamente as posições efetivas do movimento; `native` e `hidden` são opções explícitas.
- Prontidão: calibrar o relógio pelo primeiro progresso/frame do FFmpeg; não usar pré-roll fixo. Movimento, scroll e ponteiro usam deadlines monotônicos.
